...
periodic_hill(x, z) = hill(x, z) | hill(-x + Lx, z)

# zs = hill.(xs, α, H) .+ hill.(-xs .+ Lx, α, H)

FILE_DIR = "./Output"
prefix = "flow_past_periodic_hill_2D_$(solver)_Re$(Re)_alpha$(α)_Nxyz_$(Nx)x$(Ny)x$(Nz)"

x = (0, Lx)
y = (0, Ly)
z = (0, Lz)

kw = (; size=(Nx, Nz), x, z, halo=(6, 6), topology=(Periodic, Flat, Bounded))
...
vlim = (-maximum(abs, vbar_data) - 1e-3, maximum(abs, vbar_data) + 1e-3)
